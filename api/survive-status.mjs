...
import { getSurviveStatusPlaceholder } from '../server/survive/status.mjs'
import { resolveSurviveStatusForRequest } from '../server/survive/status-service.mjs'
...
export default async function handler(request, response) {
  ...
  try {
    const snapshot = await resolveSurviveStatusForRequest()
    ...
    if (snapshot.balanceUsd !== null || snapshot.holderCount !== null || snapshot.creationTimestamp !== null) {
    ...
    if (process.env.NODE_ENV !== 'production') console.error(`[vercel:status] ${error.message}`)
    // CA remains available even when every provider is unavailable.
    return response.status(200).json(getSurviveStatusPlaceholder())
  }
}
